...
export class ArrayCommand extends MultiStepCommand {
    ...
    protected resetMesh() {
        ...
        if (this.patternType === "option.command.patternType.rectangular") {
            count = this.numberX * this.numberY;
        }
    ...
    // Rows and columns only - AutoCAD's 2D ARRAY. The old third (out-of-plane) axis
    // went away with the rest of the 3D feature set.
    private getBoxTransforms(xvec: XYZ, yvec: XYZ) {
        const transforms = new Array<Matrix4>(this.numberX * this.numberY);
        let index = 0;
        for (let i = 0; i < this.numberX; i++) {
            for (let j = 0; j < this.numberY; j++) {
                const vec = xvec.multiply(i).add(yvec.multiply(j));
    ...
    private getRectangularSteps(): IStep[] {
        return [
            ...
            new PointOnAxisStep("prompt.pickNextPoint", () => this.pointOnAxisArray(), true),
        ];
    }
    ...
    protected readonly vectorArrayStepData = () => {
        return {
            ...
            preview: (p: XYZ | undefined) => {
                ...
                const matrixs = this.getBoxTransforms(vector, XYZ.zero);
    ...
    protected readonly pointOnAxisArray = () => {
        const { ray, yvec, xvec } = this.boxPlaneInfo();
        ...
        return {
            ...
            preview: (p: XYZ | undefined) => {
                ...
                const matrixs = this.boxArrayMatrixs(xvec, yvec, p);
                this.updatePosition(matrixs);
                return [
    ...
    protected boxArrayMatrixs(xvec: XYZ, yvec: XYZ, end: XYZ) {
        const x = xvec.multiply(this.stepDatas[1].point!.sub(this.stepDatas[0].point!).dot(xvec));
        const y = yvec.multiply(end.sub(this.stepDatas[0].point!).dot(yvec));
        return this.getBoxTransforms(x, y);
    }

    protected boxPlaneInfo() {
        const plane =
        ...
        // The column axis is the only remaining one - there is no out-of-plane ray.
        const ray = new Line({ point: this.stepDatas[1].point!, direction: yvec });
        return { ray, yvec, xvec };
    }
    ...
    protected getArrayTransforms() {
        switch (this.patternType) {
            ...
            case "option.command.patternType.rectangular": {
                const { xvec, yvec } = this.boxPlaneInfo();
                return this.boxArrayMatrixs(xvec, yvec, this.stepDatas[2].point!);
            }
            case "option.command.patternType.linear": {
