...
export abstract class MultistepCommand extends CancelableCommand {
    ...
    // Always the view's own workplane. The old dynamic-workplane behaviour picked one
    // of the three world planes from whatever the cursor ray hit - that only made
    // sense while the camera could orbit; a 2D drafting view draws on one plane.
    protected findPlane(view: IView, origin: XYZ, _point: XYZ | undefined) {
