...
const I18N_KEY_SET: ReadonlySet<string> = new Set(I18N_KEYS);
...
export class I18n {
    ...
    // Answers from the key list rather than from a locale, because a locale may not be
    // loaded (or may not be the one this used to hardcode - it asked "zh-CN" for a key
    // set that is language-independent, and threw once that locale stopped shipping).
    // The active locale is still consulted for keys plugins add at runtime via
    // combineTranslation, which are not in the compiled list.
    static isI18nKey(key: string): key is I18nKeys {
        if (I18N_KEY_SET.has(key)) return true;

        const language = languages.get(I18n.currentLanguage());
        return language !== undefined && key in language.translation;
    }
